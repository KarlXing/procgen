...
std::vector<const char *> WALKING_ENEMIES5 = {"wormPink"};
std::vector<const char *> PLAYER_THEME_COLORS5 = {"Yellow"};
std::vector<const char *> GROUND_THEMES5 = {"Snow"};

const int NUM_GROUND_THEMES = (int)(GROUND_THEMES5.size());

class CoinRun5 : public BasicAbstractGame {
  public:
    ...
    void load_background_images() override {
        main_bg_images_ptr = &platform_backgrounds_landscape1;
    }
    ...
    void asset_for_type(int type, std::vector<QString> &names) override {
        if (type == PLAYER) {
            for (const auto &color : PLAYER_THEME_COLORS5) {
            ...
            for (const auto &enemy : WALKING_ENEMIES5) {
            ...
            for (const auto &ground : GROUND_THEMES5) {
...
REGISTER_GAME("coinrun5", CoinRun5);