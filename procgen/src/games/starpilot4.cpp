...
const int NUM_SHIP_THEMES = 1;

bool spawn_cmp4(const std::shared_ptr<Entity> &x, const std::shared_ptr<Entity> &y) {
    return (x->spawn_time < y->spawn_time);
}

class StarPilotGame4 : public BasicAbstractGame {
  public:
    ...
    void load_background_images() override {
        main_bg_images_ptr = &space_background4;
    }
    ...
    void asset_for_type(int type, std::vector<QString> &names) override {
        if (type == PLAYER) {
            ...
            names.push_back("misc_assets/shipYellow.png");
        } else if (type == METEOR) {
    ...
    void game_reset() override {
        ...
        std::sort(spawners.begin(), spawners.end(), spawn_cmp4);
        spawn_idx = 0;
...
REGISTER_GAME("starpilot4", StarPilotGame4);
