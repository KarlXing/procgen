...
bool spawn_cmp5(const std::shared_ptr<Entity> &x, const std::shared_ptr<Entity> &y) {
    return (x->spawn_time < y->spawn_time);
}

class StarPilotGame5 : public BasicAbstractGame {
  public:
    ...
    void game_reset() override {
        ...
        std::sort(spawners.begin(), spawners.end(), spawn_cmp5);
        spawn_idx = 0;
...
REGISTER_GAME("starpilot5", StarPilotGame5);
