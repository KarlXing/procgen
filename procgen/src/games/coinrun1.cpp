...
std::vector<const char *> WALKING_ENEMIES1 = {"slimeBlock", "slimePurple"};
std::vector<const char *> PLAYER_THEME_COLORS1 = {"Beige"};
std::vector<const char *> GROUND_THEMES1 = {"Dirt"};

const int NUM_GROUND_THEMES = (int)(GROUND_THEMES1.size());

class CoinRun1 : public BasicAbstractGame {
  public:
    ...
    void load_background_images() override {
        main_bg_images_ptr = &platform_backgrounds_air1;
    }
    ...
    void asset_for_type(int type, std::vector<QString> &names) override {
        if (type == PLAYER) {
            for (const auto &color : PLAYER_THEME_COLORS1) {
            ...
            for (const auto &enemy : WALKING_ENEMIES1) {
            ...
            for (const auto &ground : GROUND_THEMES1) {
...
REGISTER_GAME("coinrun1", CoinRun1);
