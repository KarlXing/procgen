...
std::vector<const char *> WALKING_ENEMIES3 = {"mouse", "snail"};
std::vector<const char *> PLAYER_THEME_COLORS3 = {"Green"};
std::vector<const char *> GROUND_THEMES3 = {"Planet"};

const int NUM_GROUND_THEMES = (int)(GROUND_THEMES3.size());

class CoinRun3 : public BasicAbstractGame {
  public:
    ...
    void load_background_images() override {
        main_bg_images_ptr = &platform_backgrounds_cave1;
    }
    ...
    void asset_for_type(int type, std::vector<QString> &names) override {
        if (type == PLAYER) {
            for (const auto &color : PLAYER_THEME_COLORS3) {
            ...
            for (const auto &enemy : WALKING_ENEMIES3) {
            ...
            for (const auto &ground : GROUND_THEMES3) {
...
REGISTER_GAME("coinrun3", CoinRun3);
