...
std::vector<const char *> WALKING_ENEMIES4 = {"ladybug", "wormGreen"};
std::vector<const char *> PLAYER_THEME_COLORS4 = {"Pink"};
std::vector<const char *> GROUND_THEMES4 = {"Sand"};

const int NUM_GROUND_THEMES = (int)(GROUND_THEMES4.size());

class CoinRun4 : public BasicAbstractGame {
  public:
    ...
    void load_background_images() override {
        main_bg_images_ptr = &platform_backgrounds_cyberpunk1;
    }
    ...
    void asset_for_type(int type, std::vector<QString> &names) override {
        if (type == PLAYER) {
            for (const auto &color : PLAYER_THEME_COLORS4) {
            ...
            for (const auto &enemy : WALKING_ENEMIES4) {
            ...
            for (const auto &ground : GROUND_THEMES4) {
...
REGISTER_GAME("coinrun4", CoinRun4);
