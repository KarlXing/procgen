...
bool spawn_cmp1(const std::shared_ptr<Entity> &x, const std::shared_ptr<Entity> &y) {
    return (x->spawn_time < y->spawn_time);
}

class StarPilotGame1 : public BasicAbstractGame {
  public:
    ...
    void load_background_images() override {
        main_bg_images_ptr = &space_background1;
    }
    ...
    void asset_for_type(int type, std::vector<QString> &names) override {
        if (type == PLAYER) {
            ...
            names.push_back("misc_assets/shipBeige.png");
        } else if (type == METEOR) {
    ...
    void game_reset() override {
        ...
        std::sort(spawners.begin(), spawners.end(), spawn_cmp1);
        spawn_idx = 0;
...
REGISTER_GAME("starpilot1", StarPilotGame1);
